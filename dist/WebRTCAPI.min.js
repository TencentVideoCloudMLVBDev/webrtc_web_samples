...
console.debug('Version',Constants.Version)
...
var BJ_WEBRTC = __webpack_require__(37);
// 初始化badjs
if (BJ_WEBRTC) {
    var ignores = [
...
function badjsReport(_msid, _msg, level) {
    level = level || 2;
    if (BJ_WEBRTC) {
        var data = {
            ext: { msid: _msid },
...
Report.fn.quality = function () {
    ...
    if (!DetectRTC.isSafari()) {
        this.getQulityAsync(global.peerConnections, function (result, data) {
            var sendData = self.createJsonFromTag(global, Constants.RTC_EVENT.ON_QUALITY_REPORT);
            if (!result) {
...
Report.fn.endEndReport = function (endResult) {
    ...
    try {
        ...
        var sendData = this.createJsonFromTag(global, Constants.RTC_EVENT.ON_END_REPORT);
        sendData.data = {
...
var DectectRTC = {
    ...
    support: function( callback ){
        ...
        this.checkH264Support(function(support){
            ...
            ScreenChromeExtensionUtil.getChromeExtensionStatus(function( extensionResult ) {
...
var mitt = __webpack_require__(42)
var _ = __webpack_require__(43)
var jparam = __webpack_require__(45);
WebRTCStat.init();
...
function WebRTCAPI( config , succ , error ){
    var listeners = {};
    var $self = this,
        emitter = this.emitter = mitt(),
        ICE_BUILD_STATE = Constants.ICE_BUILD_STATE,
        RTC_EVENT = Constants.RTC_EVENT,
        KEY_TAG = Constants.KEY_TAG,
        MEDIA_CHANGE = Constants.MEDIA_CHANGE,
        WS_CMD = Constants.WS_CMD,
        STREAM_TYPE = Constants.STREAM_TYPE,
        STREAM_MISD = Constants.STREAM_MISD,
        USER_DEFINED_EVENT = Constants.USER_DEFINED_EVENT;
    var global =  this.global = {
        deviceInfo : {
            hasVideo : false,
            hasAudio : false
    ...
    this.onPeerConnectionAdd = function(data) {
        if( !data.srcopenid ){
            data.srcopenid = data.userId =  $self.getUserId(data.srctinyid)
        }
        if( config.peerAddNotify ){
            $self.emit('onPeerConnectionAdd',data)
            // if( listeners && listeners.onPeerConnectionAdd ){
            //     listeners.onPeerConnectionAdd(data);
            // }
        }else{
            $self.startRTC(data);
        }
    ...
    this.onStartRTCResult = function( data ){
        ...
        // $self.emit('onStartRTCResult', data);
    }
    ...
    this.createPeerConnection = function(srctinyid) {
        ...
        try {
            ...
            peerConnection.onicecandidate = function(e) {
                global.rtcUtil.onIceCandidate(global , $self.emitter, e, srctinyid);
            };
            peerConnection.onaddstream = function(e) {
                Log.RTCLog.info("peerConnection.onaddstream:" + e.stream.id);
            };
            peerConnection.oniceconnectionstatechange = function(e) {
                global.rtcUtil.onIceConnectionStateChange(global, $self.emitter, e, srctinyid);
            };
            peerConnection.ontrack = function(e) {
                Log.RTCLog.info("peerConnection.ontrack: streams.length=" + e.streams.length);
                global.rtcUtil.onAddTrack(global, $self.emitter, e.streams[0], e.track.kind,srctinyid);
            };
            peerConnection.onremovestream = function(e) {
                Log.RTCLog.info("peerConnection.onremovestream:" + e.stream.id);
                global.rtcUtil.onRemoveStream(global, $self.emitter, e.stream, srctinyid);
            };
    ...
    if( listeners ){
        listenersList.forEach(function(item){
            if( listeners[item] ){
                emitterHandler[item] = listeners[item]
            }
        })
    }
    emitter.on('*', function(type , e ){
        ...
        if( emitterHandler[type] ){
            ...
            // if( type !== 'onUserDefinedWebRTCEventNotice' ){
                // console.warn('didn\'t bind any listener for emit : ' + type)
            // }
            $self.emit(type,e)
        }
    });
    ...
    /* 
    if ( RTCPeerConnection ) {
        global.tempPeerConnection = peerConnection;
        global.tempPeerConnection.tempCreateOffer = peerConnection.createOffer( global.offerSdpOption );
    ...
    */
    ...
    var wsonmessage = function(message) {
        ...
        var cmd = resJson.cmd;
        Log.RTCLog.info("on websocket message, cmd = " + cmd);
        ...
        if (cmd === Constants.WS_CMD.CANDIDATE) {
            ...
            global.rtcUtil.onRemoteCandidate(global, $self.emitter, resJson.content, resJson.srctinyid);
            ...
            if(Constants.InternalVersion < 1001 || parseInt(resJson.srctinyid || 0) === 0){
                global.rtcUtil.onRemoteSdp(global, $self.emitter, resJson.content, resJson.srctinyid);
            }
            ...
            if (rc === 1) {
                ...
                var errorCode = global.rtcUtil.rebuildSession(global);
                if (!errorCode) {
            ...
            // if( true ){
            //     global.rtcUtil.addPeer(global , emitter, data.openid, data.tinyid, global.roomid, data.srctinyid, data.userSig, data.peerconnectionport);
            // }else{
            ...
            */
            var newConstraints = this.updateConstraints([{
                VidWidth:parseInt(data.width),
                VidHeight: parseInt(data.height),
                frameRate:  parseInt(data.frameRate)
            }] , null)
            this.onChangeConstraints(newConstraints, false, true);
        }
            ...
            global.rtcUtil.onSubVideoState(global , $self.emitter, data);
        }
            ...
            global.rtcUtil.onUpdateAudSsrc(global, $self.emitter, data);
        }
            ...
            global.rtcUtil.onUpdateVideoSsrc(global, $self.emitter, data);
        }
...
WebRTCAPI.fn.releaseUserMedia = function( media ){
...
WebRTCAPI.fn.checkInit = function(){
    var global = this.global;
    if( !global.inited ){
...
WebRTCAPI.fn.getLocalStream = function( callback) {
    var global = this.global;
    var $self = this;

    if( global.config.canvas ){
        this.pushCanvasStream( callback );
    }else if( global.config.screen ){
        var opts = {type:"audio"};
        if( global.config.screenSources ){
            opts.type = 'custom';
            opts.source = global.config.screenSources
        }
        this.getMediaByScreenShare(  opts , callback );
    }else{
        this.getMediaByCamera( callback );
    }
};


WebRTCAPI.fn.getMediaByCamera = function( callback ){
    var global = this.global;
    var $self = this;
    ...
    if( !global.constraints.video && !global.constraints.audio ) {return ;}

    global.mediaUtil.gumAndPush( { constraints : global.constraints}, callback, callback)
}

WebRTCAPI.fn.getMediaByScreenShare = function(opts, callback ){
    var global = this.global;
    var $self = this;
    global.rtcReport.send( Constants.RTC_EVENT.ON_GET_USER_MEDIA );
    ...
    ScreenChromeExtensionUtil.getScreenConstraints( opts, function(error, screen_constraints) {
        ...
        //@TODO 
        // 这里是否需要由用户指定高宽呢？
        _.extend(screen_constraints.mandatory, {
          //maxWidth:1920,// (global.constraints.video.mandatory && global.constraints.video.mandatory.maxWidth) || global.constraints.video.width.exact,
          //minWidth:1920,// (global.constraints.video.mandatory && global.constraints.video.mandatory.minWidth) || global.constraints.video.width.exact,
          //minAspectRatio: (global.constraints.video.mandatory && global.constraints.video.mandatory.minAspectRatio) || null,
          //maxAspectRatio: (global.constraints.video.mandatory && global.constraints.video.mandatory.maxAspectRatio) || null,
          maxFrameRate: (global.constraints.video.mandatory && global.constraints.video.mandatory.maxFrameRate) || global.constraints.video.frameRate || 15,
          minFrameRate: (global.constraints.video.mandatory && global.constraints.video.mandatory.maxFrameRate) || global.constraints.video.frameRate || 15
        });
        if( global.config.screenRole 
            && global.constraints.video 
            && global.constraints.video.mandatory 
            && global.constraints.video.mandatory.maxWidth ){
            screen_constraints.mandatory.maxWidth = global.constraints.video.mandatory.maxWidth
            screen_constraints.mandatory.maxAspectRatio = global.constraints.video.mandatory.maxAspectRatio
            screen_constraints.mandatory.minAspectRatio = global.constraints.video.mandatory.minAspectRatio
        ...
        if(!screen_constraints.audio) { //如果不采集音频
            global.mediaUtil.gum({constraints:screen_constraints},callback,callback);
        } else {
           global.mediaUtil.gumByScreen({constraints:screen_constraints},callback,callback);
        }
      },true);
};

WebRTCAPI.fn.pushCanvasStream = function( callback ){
    var global = this.global;
    var $self = this;

    if(global.config.canvas instanceof MediaStream){
        var stream = global.config.canvas.clone()
        ...
        Log.RTCLog.error("push canvas stream failed! canvas stream is not MediaStream");
        return false;
    }
}
...
WebRTCAPI.fn.enterRoom = WebRTCAPI.fn.createRoom = function(opts, succ, error) {
    ...
    if(typeof opts === 'object'){
        roomid = opts.roomID ||  opts.roomid;
        role = opts.role;
    }else{
        roomid = opts;
    }
    ...
    var 
        ...
        roomid = opts.roomid || global.config.roomid,
        ...
        pstnBizType = opts.pstnBizType || 0,
        pstnPhoneNumber = opts.pstnPhoneNumber || null;
        if( opts.pureAudioPush ){
            if( pstnBizType ){
                console.error('你同时定义了一个可能引起冲突的参数, pstnBizType 与 pureAudioPush不能同时定义')
                return;
            }
            if( opts.pureAudioPushAndRecord ){
                console.error('你同时定义了一个可能引起冲突的参数, pureAudioPush 与 pureAudioPushAndRecord不能同时定义')
                return;
            }
            pstnBizType = 1002
        }else if( opts.pureAudioPushAndRecord ){
            if( pstnBizType ){
                console.error('你同时定义了一个可能引起冲突的参数, pstnBizType 与 pureAudioPush不能同时定义')
                return;
            }
...
WebRTCAPI.fn.startRTC = function( opt , succ , error) {
    ...
    if( opt ){
        ...
        }else if( opt.srctinyid ){
            srctinyid = opt.srctinyid
        }
    }

    if( opt && typeof opt.canvas !== 'undefined' ){
        global.config.canvas = opt.canvas;
    }
    if( opt && typeof opt.screen !== 'undefined' ){
        global.config.screen = opt.screen;
        if( opt.screenSources ){
            global.config.screenSources = opt.screenSources;
        }
    }
    if( opt && opt.screenRole ){
        global.config.screenRole = opt.screenRole
    }
    if( succ ){
    ...
    if (!this.createPeerConnection(srctinyid || 0)) {
    ...
    if (parseInt(srctinyid || 0) === 0) {
        ...
        this.getLocalStream(function (data) {
            ...
            global.rtcUtil.getSdp(global , $self.emitter, srctinyid, function(errorCode, info) {
                if (errorCode !== 0) {
                ...
                global.rtcUtil.sendSdp(global, $self.emitter, info, srctinyid);
                $self.emitter.emit("onStartRTCResult",{
                    errorCode: 0,
                    errorMsg: "ok",
                    srctinyid : srctinyid
        ...
        console.debug( Constants.InternalVersion )
        if( Constants.InternalVersion > 1000 ){
            global.rtcUtil.onRemoteSdp(global, $self.emitter, opt.remoteSdp, srctinyid);
            global.rtcUtil.getAnswerSdp(global , $self.emitter, srctinyid, function(errorCode, info) {
                if (errorCode !== 0) {
...
WebRTCAPI.fn.quit = function(succ , error) {
    try{
        ...
        global.rtcUtil.clearGlobalValues( global );
...
WebRTCAPI.fn.changeSpearRole = function(role) {
    var global = this.global;
...
WebRTCAPI.fn.onChangeConstraints = function(newConstraints, isChangeAudio, isChangeVideo) {
    ...
    var srctinyid = 0;
    global.rtcUtil.updateLocalStream(global, emitter,  newConstraints, isChangeAudio, isChangeVideo, function(errorCode, media) {
      if(errorCode === 0) { // 没有异常
      ...
      global.rtcUtil.getSdp(global , emitter,srctinyid, function(errorCode, info) {
          if (errorCode !== 0) {
          ...
          global.rtcUtil.sendUpdateSdp(global, emitter, info, srctinyid);
      });
    });
  };
...
WebRTCAPI.fn.detectRTC = function( callback ){
    return DetectRTC.support( callback );
}
...
WebRTCAPI.fn.getStats = function ( opts , succ , error) {
    ...
    if( global.peerConnections[tinyId] ){
        global.peerConnections[tinyId].getPeerStats( succ, interval);
    }else{
...
RTCUtil.fn.isSupportH264 = function (sdp) {
  ...
  RTCUtil.fn.filterIceCandidate = function (candidate) {
    var str = candidate.candidate;
    if (str.indexOf("tcp") !== -1) {
      return false;
    }
    return true;
    // var type = this.getIceCandidateType(candidate);
    // if (type !== "srflx") {
    //   return false;
    // }
    // return true;
...
RTCUtil.fn.getSdp = function (global, emitter, srctinyid, callback) {
  var self = this
  if (!srctinyid) {
    srctinyid = 0;
  }
...
RTCUtil.fn.updateRemoteSdp = function (global, emitter, srctinyid, newSdp) {
  // Log.RTCLog.info("onUpdateRemoteSdp: srctinyid" + srctinyid + "  newSdp:" + newSdp.sdp);
...
RTCUtil.fn.onSubVideoState = function (global, emitter, data) {
  ...
  if (!peerConnection) {
    var errorMsg =
      "onSubVideoState, failed to get peerconnection. srctinyid = " +
      data.srctinyid;
    Log.RTCLog.error(errorMsg);
    return;
  }
  ...
  this.updateRemoteSdp(global, emitter, data.srctinyid, data.newSdp);
};

RTCUtil.fn.onUpdateAudSsrc = function (global, emitter, data) {
...
RTCUtil.fn.onUpdateVideoSsrc = function (global, emitter, data) {
...
RTCUtil.fn.rebuildSession = function (global) {
  try {
    console.log(
      "begin rebuild session!!!  size = " +
      global.websocket.sessioninfo.srcids.length
    );
    var sendData = this.global.rtcReport.createJsonFromTag(
...
RTCUtil.fn.onIceCandidate = function (global, emitter, e, srctinyid) {
  ...
  if (!candidate) {
    ...
    if (pc.localCandidateList.length <= 0) {
      //candidate 失败
      Log.RTCLog.error("get local candidate failed!");
      emitter.emit("onUserDefinedWebRTCEventNotice", {
    ...
    if (pc.isSdpSendOK && !pc.hasSendCandidate) {
      Log.RTCLog.info("send candidate when ice end!");
      pc.hasSendCandidate = true;
      var len = pc.localCandidateList.length;
      var c = pc.localCandidateList[len - 1];
      this.sendLocalCandidate(global, emitter, c, srctinyid);
    }
    return;
  }

  Log.RTCLog.info(
    "on ice candidate : sdpMLineIndex = " +
  ...
  if (this.filterIceCandidate(candidate)) {
    var msg = {
    ...
    if (pc.localCandidateList.length > 0 && pc.isSdpSendOK && !pc.hasSendCandidate) {
      pc.hasSendCandidate = true;
      this.sendLocalCandidate(
        global,
        emitter,
        pc.localCandidateList[pc.localCandidateList.length - 1],
        srctinyid
      );
    }
  } else {
    Log.RTCLog.warn(
      "filterIceCandidate failed",
      "Only UDP protocol is supported [ it is just an ability-check , doesn't mean u can not connect ,wait for next ice callback]",
      candidate.candidate
    );
  }
};

RTCUtil.fn.sendSdp = function (global, emitter, sdp, srctinyid) {
  var sendData = this.global.rtcReport.createJsonFromTag( Constants.RTC_EVENT.ON_PEER_SDP);
  sendData.data = sdp;
  sendData.srctinyid = srctinyid;
  global.websocket.send(JSON.stringify(sendData));
};

RTCUtil.fn.sendUpdateSdp = function (global, emitter, sdp, srctinyid) {
  var sendData = this.global.rtcReport.createJsonFromTag(
...
RTCUtil.fn.onAddTrack = function (global, emitter, stream, type, srctinyid) {
  Log.RTCLog.info(
  ...
  if (srctinyid == "1") {
    Log.RTCLog.info(
      "onAddTrack, srctinyid:" + srctinyid + ", data channel, not show video!"
    );
    return;
  }
  if (srctinyid != "0") {
...
RTCUtil.fn.onIceConnectionStateChange = function (global, emitter, e, srctinyid) {
  emitter.emit(
    "onUserDefinedWebRTCEventNotice", {
...
RTCUtil.fn.onRemoveStream = function (global, emitter, stream, srctinyid) {
  ...
  navigator.mediaDevices
    ...
    .then(function (media) {
      ...
      if (isChangeVideo) {
        ...
        global.mediaUtil.reformStream(global.localStream, global.localMediaStatus);
      }
      callback(0, media);
    })
    .catch(function (error) {
      navigator.getUserMedia(
        ...
        function (error) {
          var errorMsg =
            "get new user media failed : error = " + error.message;
          Log.RTCLog.error(errorMsg);
...
RTCUtil.fn.onRemoteCandidate = function (global, emitter, remoteCandidate, srctinyid) {
  var self = this;
  Log.RTCLog.info(
  ...
  peerConnection.addIceCandidate(
    ...
    function (e) {
      ...
      global.rtcReport.send( Constants.RTC_EVENT.ON_SET_REMOTE_CANDIDATE_SUC);
      emitter.emit('onIceConnectionBuild', {
        result: true,
...
RTCUtil.fn.sendLocalCandidate = function (global, emitter, candidate, srctinyid) {
  if (candidate) {
    var sendData = this.global.rtcReport.createJsonFromTag(
...
RTCUtil.fn.onRemoteSdp = function (global, emitter, remoteSdp, srctinyid) {
  Log.RTCLog.info(
    "srctinyid:" + srctinyid + "on remote sdp : " + remoteSdp.sdp
  );
  srctinyid = global.config.tinyid === srctinyid ? 0 : srctinyid;
  var peerConnection = global.peerConnections[srctinyid];
  var self = this;
    ...
    function () {
      if (peerConnection.localCandidateList.length > 0 && !peerConnection.hasSendCandidate) {
        ...
        self.sendLocalCandidate(
          global,
          emitter,
          peerConnection.localCandidateList[peerConnection.localCandidateList.length - 1],
          srctinyid
        );
      }
      emitter.emit("onUserDefinedWebRTCEventNotice", {
...
RTCUtil.fn.addPeer = function (
  ...
  this.global.rtcReport.send( Constants.RTC_EVENT.ON_CREATE_PEER, data )
  return true;
};

RTCUtil.fn.clearGlobalValues = function (global) {
  global.config = {
    sdkAppId: "",
    openid: "",
    tinyid: "",
...
RTCUtil.fn.getAnswerSdp = function (global, emitter, srctinyid, callback) {
  if (!srctinyid) {
    srctinyid = 0;
  }
  var peerConnection = global.peerConnections[srctinyid];
...
MediaUtil.fn.gum = function( opts, succ, fail ){
    ...
    navigator.mediaDevices.getUserMedia( opts.constraints ).then(function (stream) {
        ...
        if (self.hasRetryOpenCamera) {
            if( global ){
                var errorMsg = "get user media failed : error = " + error.message + " global.constraints:" + JSON.stringify(opts.constraints);
                Log.RTCLog.error(errorMsg);
                Log.uploadWebLog(global, errorMsg, null, Constants.KEY_TAG.RTC_LOG_TAG);
            }
            fail && fail({
            ...
            if( global ){
            var errorMsg = "get user media failed, error = " + error.message  + " opts.constraints:" + JSON.stringify(opts.constraints) + "; RetryOpenCamera";
...
MediaUtil.fn.gumByScreen = function(opts, succ, fail){
    ...
    Promise.all([navigator.mediaDevices.getUserMedia(_.extend({},screen_constraints,{audio:false})), navigator.mediaDevices.getUserMedia({audio:true})]).then(function(streams) {
...
MediaUtil.fn.gumAndPush = function(opts, succ, fail){
        var global = this.global
        this.gum( opts, function( data ){
            ...
            global.rtcReport.send( Constants.RTC_EVENT.ON_GET_USER_MEDIA_OK, global.constraints);
            succ({
                errorCode:0,
                data:stream
            });
