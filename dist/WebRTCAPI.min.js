    ...
    var wsonmessage = function(message) {
        ...
        if (cmd === Constants.WS_CMD.CANDIDATE) {
            ...
            */
            ...
            console.error('onChangeConstraints',newConstraints)
...
WebRTCAPI.fn.getLocalStream = function(opts, callback) {
    ...
    if( opts && opts.screen ){
        ...
        this.getMediaByCamera( opts, callback );
    }
};


WebRTCAPI.fn.getMediaByCamera = function( opts, callback ){
    var global = this.global;
    ...
    global.constraints = global.mediaUtil.packConstraints(global.constraints, opts , { frameRate : 20} )
    
    

    if( !global.constraints.video && !global.constraints.audio ) {return ;}

    global.mediaUtil.gumByMediaDevice( { constraints : global.constraints}, callback, callback)
}
...
WebRTCAPI.fn.getMediaByScreenShare = function(opts, callback ){
    ...
    ScreenChromeExtensionUtil.getScreenConstraints( opts, function(error, screen_constraints) {
        ...
        var final_constraints = {
          audio: screen_constraints.canRequestAudioTrack,
        ...
        if( opts.screenSources.indexOf("audio") === -1){
        ...
        global.mediaUtil.gumByScreen({constraints:final_constraints},callback,callback);
      },true);
};
...
WebRTCAPI.fn.startRTC = function( opt , succ , error) {
    ...
    //兼容现有用户的代码
    opt.role = opt.role || opt.screenRole
    if( opt.role ){
...
RTCUtil.fn.onIceCandidate = function (e, srctinyid) {
  ...
  if (!candidate) {
    ...
    if (pc.localCandidateList.length <= 0) {
      ...
      global.rtcReport.send( Constants.RTC_EVENT.ON_GET_LOCAL_CANDIDATE_FAILED, {
        ...
        candidateList: pc.candidateList
      });
      return;
    }
    ...
    if (pc.isSdpSendOK && !pc.hasSendCandidate) {
      ...
      this.sendLocalCandidate(c, srctinyid);
    }
    return;
  }
  ...
  if( candidate && candidate.candidate ){
    pc.candidateList.push(candidate.candidate);
  }
...
RTCUtil.fn.sendLocalCandidate = function (candidate, srctinyid) {
  var global = this.global
  if (candidate) {
    var sendData = global.rtcReport.createJsonFromTag(
...
MediaUtil.fn.gumByScreen = function(opts, succ, fail){
    ...
    if( screen_constraints.audio){
        promises.push( Promise.resolve( navigator.mediaDevices.getUserMedia({audio:true}) ) )
    }
...
MediaUtil.fn.packConstraints = function( constraints, opts, ext){
    ...
    if (DetectRTC.isSafari()) {
        ...
        if( opts && opts.audioDevice && opts.audioDevice.deviceId ){
            // constraints.audioDevice.deviceId = opts.audioDevice.deviceId
        ...
        if( opts && opts.videoDevice && opts.videoDevice.deviceId){
            if( Constants.ConstraintsMode === 1 ){
                if( typeof constraints.video === 'object'){
                    constraints.video.deviceId = opts.videoDevice.deviceId;
                }else{
                    constraints.video.deviceId = opts.videoDevice.deviceId
                    constraints.video = {
                ...
                if( typeof constraints.video === 'object'){
                    constraints.video.mandatory.sourceId =  opts.videoDevice.deviceId;
                }else{
                    constraints.video = {
                        mandatory: {
    ...
    if (constraints.video && opts && opts.attributes) {
        if( !constraints.video.mandatory ){
            constraints.video.mandatory = {};
        }
        constraints.video.mandatory.maxWidth = (opts.attributes && (opts.attributes.maxWidth || opts.attributes.width)) ||  null
        constraints.video.mandatory.minWidth =  (opts.attributes && (opts.attributes.minWidth || opts.attributes.width)) || null
        constraints.video.mandatory.maxHeight = (opts.attributes && (opts.attributes.maxHeight || opts.attributes.height)) ||  null
        constraints.video.mandatory.minHeight =  (opts.attributes && (opts.attributes.minHeight || opts.attributes.height)) || null
        constraints.video.mandatory.maxFrameRate = (opts.attributes && (opts.attributes.maxFrameRate || opts.attributes.frameRate)) ||  ext.frameRate || null
        constraints.video.mandatory.minFrameRate =  (opts.attributes && (opts.attributes.minFrameRate || opts.attributes.frameRate)) ||  ext.frameRate || null
        for(var a in constraints.video.mandatory){
            if(!constraints.video.mandatory[a]){
                delete constraints.video.mandatory[a];
            }
        }
    }
    return constraints;
